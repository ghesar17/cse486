...
type Simulator struct {
	...
	logger         *Logger
	snapFinished   *SyncMap // key = server ID : value = bool
	snapshots	   *SyncMap // key = server ID : value = snapshot
}
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	sim.snapFinished.Store(serverId, false)
	server := sim.servers[serverId]
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// set the snapshot to finished
	sim.snapFinished.Store(serverId, true)

	// store the server's snapshot
	// server := sim.servers[/serverId]
	// sim.snapshots.Store(serverId, server.snapshot)
}

// Collect and merge snapsshot state from all the servers.
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	...
	for serverID, server := range sim.servers {
		...
		serverSnapshot, _ := server.snapshot.Load(serverID)
		serverSnap := serverSnapshot.(SnapshotState)
		snap.tokens[serverID] = serverSnap.tokens[serverID]
		snap.messages = serverSnap.messages
	}
