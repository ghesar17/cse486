...
func mapF(document string, value string) (res []mapreduce.KeyValue) {
	...
	for _, word := range wordArray {
		_, in := wordMap[word]
		// if havent seen word before add to map
		if !in {
			wordMap[word] = mapreduce.KeyValue{Key: word, Value: document}
		} 
	}
...
func reduceF(key string, values []string) string {
	// take the documents the word was found in and concat into a string
	...
	// 	allDocs += document 
	// }
	// lengthInt := strconv.Itoa(len(values))

	// return lengthInt + " " + allDocs
